...
"""
可作为 remote_sim 轻量模拟器侧服务运行的客户端脚本。

中文导读：
这个文件是 Action-aware GRPO 的模拟器侧客户端/服务封装。它复用在线闭环协议：
同一个 `session_id` 先发 1 帧预热图像，再按 `step` 上传执行动作后得到的真实新帧。
当前 `tsformer_latent` Stage-2 服务端返回 16 个逐帧动作；旧 4 个宏动作输出
仍保留兼容处理。
...
def main():
    ...
    ap.add_argument("--out_dir", type=str, default=os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "outputs", "runtime_cache")), help="每个 session 的 JSON 输出目录。")
