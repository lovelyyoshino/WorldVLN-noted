...
运行示例：
  ...
  uvicorn pipeline_test.infinity_tsformer_api_server:app --host 0.0.0.0 --port 8002

自测示例（需要真实 ckpt 与 route_dir）：
  python3 pipeline_test/infinity_tsformer_api_server.py --self_test \
...
TSFORMER_ROOT = ROOT / "TSformer-VO-main"
...
# -------------------------
# NOTE: InfinityStar repo is selected at runtime so this server can embed different
# copies (e.g. pipeline_test/InfinityStar-main vs repo-root/InfinityStar-main).
DEFAULT_INFINITY_REPO_ROOT = ROOT / "InfinityStar-main"
...
DEFAULT_ACTIONHEAD_REPO_ROOT = REPO / "actionhead" / "TSformer-VO-main" / "TSformer-VO-main"
