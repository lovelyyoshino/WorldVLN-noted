...
INFINITY_API_PY="${INFINITY_API_PY:-${PACKAGE_ROOT}/infinity_tsformer_api_server.py}"
INFINITY_SERVER_CONFIG="${INFINITY_SERVER_CONFIG:-${PACKAGE_ROOT}/config.json}"
INFINITY_CKPT="${INFINITY_CKPT:-}"
INFINITY_REPO_ROOT="${INFINITY_REPO_ROOT:-${ROOT_DIR}}"
CHECKPOINTS_DIR="${CHECKPOINTS_DIR:-${PACKAGE_ROOT}/models/infinity}"
...
ACTIONHEAD_REPO_ROOT="${ACTIONHEAD_REPO_ROOT:-${PACKAGE_ROOT}/TSformer-VO-main}"
...
if [ "${UAVFLOW_STAGEA_ROLLOUT_BACKEND}" != "remote_sim" ]; then
  ...
  echo "[stageA][fatal] open-source posttrain only keeps remote_sim for StageA."
  exit 1
fi
