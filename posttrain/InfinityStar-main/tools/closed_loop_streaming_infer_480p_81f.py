...
"""
闭环/流式推理（离线回放 + 可选实时 watch）：

- 总预测长度固定为 N 帧（默认 81）。
- 第一次仅输入真实第 1 帧 + prompt，预测到 N 帧。
- 之后每次“前进 step 帧”（默认 16），在写入下一段真实帧缓存前，先把“下一段 step 帧预测”
  分段保存；然后清除 Pred cache，把累计真实历史帧写入 GT cache，再继续推理。
- 直到累计真实帧达到 min(N, 实际可用真实帧数)。

你只需要准备一个按时间顺序命名的真实帧目录（例如 000000.png ...），脚本会自动排序读取。

示例（你的 49 帧需求）：
  python3 tools/closed_loop_streaming_infer_480p_81f.py \
        --ckpt /path/to/infinity/global_step_xxx.pth \
        --route_dir /path/to/reference_route \
    --num_frames 49 --step 16 --prompt_key instruction \
        --out_dir ./outputs/closed_loop
"""
...
PACKAGE_ROOT = osp.abspath(osp.join(REPO_ROOT, ".."))
...
def _make_args(
    *,
    ...
    # Prefer shared pretrained assets used by finetune; allow env override.
    checkpoints_dir = os.environ.get("CHECKPOINTS_DIR", osp.join(PACKAGE_ROOT, "models", "infinity"))
    ...
    # ---------------------------------------------------------------------
    # IMPORTANT: In some envs Args(Tap) can't be instantiated; we fall back
    # to SimpleNamespace. In that case we must provide all fields that are
    # touched by schedules / video_encode / self_correction during inference.
    # These defaults mirror infinity/utils/arg_util.py.
    # ---------------------------------------------------------------------
    _set_default("device", "cuda")
    _set_default("reduce_accumulate_error_method", "bsc")

    # ----------------------------
    # Scale / schedule (critical)
    # ----------------------------
    _set_default("min_scale_ind", 3)
    _set_default("max_reweight_value", 40)
    _set_default("inner_scale_boost", 0)
    _set_default("drop_720p_last_scale", 1)
    _set_default("reweight_loss_by_scale", 0)
    _set_default("image_scale_repetition", "[3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3]")
    _set_default("video_scale_repetition", "[3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 2, 1]")

    # ----------------------------
    # Model config (safe defaults)
    # ----------------------------
    _set_default("sdpa_mem", True)
    _set_default("rms_norm", False)
    _set_default("tau", 1.0)
    _set_default("tini", -1.0)
    _set_default("topp", 0.0)
    _set_default("topk", 0.0)
    _set_default("fused_norm", False)
    _set_default("flash", False)
    _set_default("use_flex_attn", True)
    _set_default("norm_eps", 1e-6)
    _set_default("Ct5", 2048)
    _set_default("simple_text_proj", 1)
    _set_default("mask_video_first_frame", 0)
    _set_default("rope_type", "4d")
    _set_default("rope2d_each_sa_layer", 1)
    _set_default("rope2d_normalized_by_hw", 2)
    _set_default("add_lvl_embeding_on_first_block", 0)
    _set_default("use_learnable_dim_proj", 0)
    _set_default("use_feat_proj", 2)
    _set_default("pad_to_multiplier", 128)
    _set_default("apply_spatial_patchify", 0)
    _set_default("noise_input", 0)
    _set_default("max_repeat_times", 10000)
    _set_default("semantic_scales", 11)
    _set_default("semantic_scale_dim", 16)
    _set_default("detail_scale_dim", 64)
    _set_default("detail_scale_min_tokens", 350)
    _set_default("use_two_stage_lfq", 1)

    # ----------------------------
    # Self-correction (required)
    # ----------------------------
    _set_default("noise_apply_layers", 1000)
    _set_default("noise_apply_requant", 1)
    # In inference we do NOT want to inject noise into GT indices; keep it disabled.
    # (Args default is '-1', which effectively results in 0 noise.)
    _set_default("noise_apply_strength", "-1")
    _set_default("noise_apply_random_one", 0)
    _set_default("num_of_label_value", 2)
    _set_default("noise_apply_requant", 1)
    _set_default("debug_bsc", 0)

    # ----------------------------
    # Data / video basics
    # ----------------------------
    _set_default("video_fps", int(fps))
    _set_default("fps", int(fps))
    _set_default("video_frames", int(num_frames))
    _set_default("video_sample_mode", "fixed_full")
    _set_default("video_batch_size", 1)
    _set_default("image_batch_size", 0)
    _set_default("workers", 8)
    _set_default("tlen", 512)
    _set_default("duration_resolution", 1)
    _set_default("seq_pack_bucket", 1000)
    _set_default("drop_long_video", 0)
    _set_default("min_video_frames", -1)
    _set_default("allow_less_one_elem_in_seq", 1)

    # ----------------------------
    # Context defaults
    # ----------------------------
    _set_default("context_frames", 10000)
    _set_default("context_interval", 2)
    _set_default("context_from_largest_no", 0)
    _set_default("cached_video_frames", int(getattr(a, "video_frames", num_frames)))
    _set_default("frames_inner_clip", 4)

    # ----------------------------
    # VAE basics (required)
    # ----------------------------
    _set_default("vae_type", 64)
    _set_default("videovae", 10)
    _set_default("temporal_compress_rate", 4)
    _set_default("use_slice", 1)
    _set_default("use_vae_token_cache", 1)
    _set_default("save_vae_token_cache", 0)
    _set_default("allow_online_vae_feature_extraction", 1)
    _set_default("use_text_token_cache", 0)

    # ----------------------------
    # Distributed / misc (safe)
    # ----------------------------
    _set_default("sp_size", 0)
    _set_default("inject_sync", 0)
    a.pn = pn
    a.fps = int(fps)
    ...
    a.videovae = 10
    a.vae_type = 64
    a.vae_path = osp.join(checkpoints_dir, "infinitystar_videovae.pth")
    a.text_encoder_ckpt = osp.join(checkpoints_dir, "text_encoder", "flan-t5-xl-official")
    ...
    # Align with clip4 finetuning setup.
...
def main() -> None:
    ...
    ap.add_argument(
        "--ckpt",
        type=str,
        default=os.environ.get("INFINITY_CKPT", ""),
        help="global_step_*.pth（checkpoint_type=torch）",
    )
    ...
    ap.add_argument("--out_dir", type=str, default=osp.join(PACKAGE_ROOT, "outputs", "closed_loop"))
    ...
    ap.add_argument("--strong_history_leak", action="store_true", default=True, help="Use all observed real frames as forced leak each segment")
    ap.add_argument("--no_strong_history_leak", action="store_false", dest="strong_history_leak")
    ap.add_argument("--infer_to_next_obs", action="store_true", default=True, help="Per segment infer only up to next obs point")
    ap.add_argument("--no_infer_to_next_obs", action="store_false", dest="infer_to_next_obs")

    ap.add_argument("--dynamic_scale_schedule", type=str, default="infinity_elegant_clip4frames_v2_allpt")
    ap.add_argument("--mask_type", type=str, default="infinity_elegant_clip4frames_v2_allpt")
    ...
    # ------------------------------------------------------------
    # Build helpers for GT leak constraints
    # ------------------------------------------------------------
    self_correction = SelfCorrection(vae, a)
    video_encode, _, _, _ = get_encode_decode_func(a.dynamic_scale_schedule)

    first_p = frame_paths[0]
    ref = transform(Image.open(first_p).convert("RGB"), tgt_h, tgt_w)  # [3,H,W] in [-1,1]
    ref_T3HW = torch.stack([ref], dim=0)  # [1,3,H,W]
    ref_bcthw = ref_T3HW.permute(1, 0, 2, 3).unsqueeze(0).to("cuda")  # [1,3,1,H,W]
    dyn_res_1f, _ = get_dynamic_resolution_meta(a.dynamic_scale_schedule, 1)
    _, _, gt_ls_Bl_first, _, _, _ = video_encode(
        vae=vae,
        inp_B3HW=ref_bcthw,
        vae_features=None,
        ...
        args=a,
        infer_mode=True,
        dynamic_resolution_h_w=dyn_res_1f,
    )
    sched_1f = session.build_schedule_for_num_frames(num_frames=1)
    gt_leak_first = int(len(sched_1f.scale_schedule))
        ...
        json.dump(
            {
                ...
                "strong_history_leak": bool(args_cli.strong_history_leak),
                "infer_to_next_obs": bool(args_cli.infer_to_next_obs),
                "prompt": prompt,
    ...
    def _infer_full(step_i: int, *, obs_len: int, target_num_frames: int, obs_for_leak: torch.Tensor) -> torch.Tensor:
        """Infer full N frames, return uint8 BGR tensor [T,H,W,3] (first sample)."""
        seed = int(args_cli.seed) + int(step_i)
        t0 = time.time()
        sched_cur = session.build_schedule_for_num_frames(num_frames=int(target_num_frames))
        tau_cur = [float(a.tau_image)] * int(sched_cur.tower_split_index) + [float(a.tau_video)] * (len(sched_cur.scale_schedule) - int(sched_cur.tower_split_index))

        if bool(args_cli.strong_history_leak):
            dyn_res_obs, _ = get_dynamic_resolution_meta(a.dynamic_scale_schedule, int(obs_len))
            _, _, cur_gt_ls_Bl, _, _, _ = video_encode(
                vae=vae,
                inp_B3HW=obs_for_leak,
                vae_features=None,
                self_correction=self_correction,
                args=a,
                infer_mode=True,
                dynamic_resolution_h_w=dyn_res_obs,
            )
            sched_obs = session.build_schedule_for_num_frames(num_frames=int(obs_len))
            cur_gt_leak = int(len(sched_obs.scale_schedule))
        else:
            cur_gt_leak = int(gt_leak_first) if int(step_i) == 0 else -1
            cur_gt_ls_Bl = gt_ls_Bl_first if int(step_i) == 0 else None
    ...
    if not args_cli.watch:
        ...
        for i in range(len(points) - 1):
            ...
            target_nf = int(next_obs_len) if bool(args_cli.infer_to_next_obs) else int(args_cli.num_frames)
            padded_obs = _take_with_pad(frame_paths, int(obs_len), bool(args_cli.pad_short_real))
            obs_for_leak = _load_obs_video_bcthw(padded_obs, tgt_h, tgt_w).to("cuda", non_blocking=True)
            vid = _infer_full(step_i=i, obs_len=obs_len, target_num_frames=target_nf, obs_for_leak=obs_for_leak)
