...
class CKPTSaver(object):
    ...
    def sav(
        ...
        print(f'checkpoint 已保存到: {local_out_ckpt}', flush=True)
...
def auto_resume(args: arg_util.Args, pattern='*.pth') -> Tuple[List[str], int, int, str, List[Tuple[float, float]], dict, dict]:
    ...
    if args.auto_resume:
        ...
        if len(all_ckpt) == 0:
            info.append(f'[auto_resume] 未找到 checkpoint @ {pattern}')
    ...
    print(f'从 {resume} 自动恢复 checkpoint')
    ...
    def sav(
        ...
        if dist.is_master():
            ...
            def auto_sync(source_filename, target_filename):
                ...
                print(f'[Saver] 自动备份日志命令 auto_save cmd: {cmd}', flush=True)
            ...
            cost = time.time() - stt
        print(f'[CKPTSaver][rank00][omnistore: {FSDPCheckpointer is not None}] 耗时={time.time()-stt:.2f}s, checkpoint 已保存到 global_step_{global_it}', flush=True)
...
def merge_ckpt(omnistore_ckpt_path, output_path, fsdp_save_flatten_model, save=False):
    ...
    if 'semantic_head.weight' in state_dict_model:
        print(f'[rush_resume] 将 semantic_head 权重映射到 semantic_head2')
