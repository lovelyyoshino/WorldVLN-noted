...
class EuclideanCodebook(nn.Module):
    ...
    def update_affine(self, data, embed, mask = None):
        ...
        # 中文说明：准备 batch 数据，具体取决于是否有 mask
...
class VectorQuantize(nn.Module):
    ...
    def __init__(
        ...
        sample_codebook_temp = 1.,
        straight_through = False,
        reinmax = False,  # 中文说明：使用 reinmax 改进 straight-through（前提是 straight-through 有帮助）
        sync_codebook = None,
        sync_affine_param = False,
        ema_update = True,
        learnable_codebook = False,
        in_place_codebook_optimizer: Callable[..., Optimizer] = None, # 中文标题：使用 learnable_codebook 时用于更新 codebook embedding 的优化器
        affine_param = False,
        affine_param_batch_decay = 0.99,
        affine_param_codebook_decay = 0.9,
        ...
        self.commitment_use_cross_entropy_loss = commitment_use_cross_entropy_loss # 中文标题：是否使用到 codebook 的 cross entropy loss 作为 commitment loss
    ...
    def forward(
        ...
        # 中文标题：如果要返回传入 codes 上的 cross entropy loss
        ...
        # 代码/形状说明：如果有 mask，只在 mask 为 True 的位置返回 quantized
