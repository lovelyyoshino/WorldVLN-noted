...
def get_timestep_embedding(dim, timesteps=1000, max_period=10000):
    ...
    """
    assert dim % 2 == 0, "embedding 维度必须是偶数"
    half = dim // 2
...
class Infinity(nn.Module):
    ...
    def __init__(
        ...
        qwen_qkvo_bias=False,               # Qwen 注意力里的 q/k/v/o 是否带 bias
        ...
        cond_drop_rate=0.1,                 # 无分类器引导（CFG）的条件丢弃概率
        rand_uncond=False,
        drop_path_rate=0.1,
        ...
        self.raw_scale_schedule = raw_scale_schedule    # raw 表示还没有经过空间分块的原始尺度表
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC: torch.Tensor,
        ...
        **kwargs,
        ...
        """
        训练前向传播：把视觉 token 和文本 token 拼成一个序列后计算监督 loss。

        `label_B_or_BLT` 在本项目里通常是 compact 文本 tuple：
        注意力输入元组：`(kv_compact, lens, cu_seqlens_k, max_seqlen_k)`。
        返回逐视觉 token 的 loss、accuracy，以及 padding 后的有效 token 比例。

        建议阅读顺序：
        1. 先看“怎样把视觉 token 和文本 token 拼成一个训练序列”；
        2. 再看 Transformer block 如何消费这个拼接序列；
        3. 最后看为什么只取视觉 token 计算 loss。

        这里的训练目标不是“重建整句文本”，而是：
        给定文本条件和历史视觉 token，预测接下来的视觉 latent bit label。
        """
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            # 训练时随机把部分文本条件替换成无条件 token，用于 CFG。
            # 直觉上就是：让模型学会“有条件”和“无条件”两种前向，
            # 推理时才能用 cond/uncond 做 CFG 组合。
            total = 0
            for le in lens:
            ...
            # 因为 Transformer 主体只认统一维度的 token 序列，不区分“这是文本”还是“这是视觉”。
        ...
        # 到这里为止，模型看到的是“视觉 token 前缀 + 文本 token 条件”的统一序列。
        ...
        # 原因是：文本 token 只是条件，不是当前训练任务的监督目标。
    ...
    def prepare_text_conditions(
        ...
        输出的 `prefix_tokens` 会在后续推理里以 cache key `t0` 写入 KV cache，
        相当于“整条轨迹都共享的一份语言条件记忆”。
        """
        kv_compact, lens, cu_seqlens_k, max_seqlen_k = label_B_or_BLT
        bs = B
    ...
    def ar_infer_infinity_elegant(
        ...
        **kwargs,
    ):   # 返回采样 token 或解码后视频，取决于 return_summed_code_only。
        """
        ...
        小白直觉版：
        - `forward()` 是“老师把正确答案喂给你，你学着预测下一步”；
        - `ar_infer_infinity_elegant()` 是“模型自己一步一步往前写 latent 未来”。
        WorldVLN 在线服务真正依赖的是这里生成的 `summed_codes`，而不是一定要把它们解码成视频。
        """
        ...
        for si, pn in enumerate(scale_schedule):   # si 表示当前尺度/clip 内位置。
            ...
            for repeat_idx in range(infer_repeat_times):
                ...
                # 接下来要把“离散 bit label”再映射回连续 latent code，才能累计到 summed_codes。
                if self.other_args.use_two_stage_lfq:
    ...
    def ar_infer_infinity_star_interact(
        ...
        **kwargs,
        ...
        for b in self.unregistered_blocks: b.attn.kv_caching(False, reset=kv_cache_reset)
        if mode == 'second_v_clip':
            # 把压缩时间长度换算回像素帧数，用于截取当前 clip 的 decoded video。
            ...
            img = self.summed_codes2images(vae, summed_codes) # 解码后视频，形状 [bs, t, h, w, 3]
    ...
    def autoregressive_infer_cfg(
        ...
        **kwargs,
        ...
        ret, idx_Bl_list = [], []  # 兼容旧返回格式的占位列表。
        for b in self.unregistered_blocks: b.attn.kv_caching(True)
        summed_codes = 0
        for si, pn in enumerate(scale_schedule):   # si 表示当前尺度编号。
            ...
            attn_mask = torch.ones((last_stage.shape[0], 1, last_stage.shape[1], text_maxlen_this_iter+np.array(pn).prod()), device=last_stage.device).bool() # [bs, q_heads, q_len, all_k_len]，这里 q_heads=1 方便广播。
    ...
    def summed_codes2images(self, vae, summed_codes):
        ...
        print(f'解码耗时 {time.time()-t1:.1f}s')
        return img
