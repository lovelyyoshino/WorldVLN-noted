...
# 用于计算 precise BN 的批量数量。
_C.BN.NUM_BATCHES_PRECISE = 200

# 应用于 BN 参数的权重衰减。
_C.BN.WEIGHT_DECAY = 0.0

# 归一化类型，可选 `batchnorm`、`sub_batchnorm`、`sync_batchnorm`。
_C.BN.NORM_TYPE = "batchnorm"

# SubBatchNorm 参数：把批量维度切成 NUM_SPLITS 份，并分别独立运行 BN。
_C.BN.NUM_SPLITS = 1
...
# 组数量；ResNet 为 1，ResNeXt 通常大于 1。
_C.RESNET.NUM_GROUPS = 1

# 每个组的通道宽度（64 -> ResNet；4 -> ResNeXt）。
_C.RESNET.WIDTH_PER_GROUP = 64

# 是否以原地方式执行 ReLU。
_C.RESNET.INPLACE_RELU = True

# 是否把 stride 放在 1x1 conv 上。
...
# 如果当前模块数超过 NUM_BLOCK_TEMP_KERNEL，剩余模块使用 temporal kernel=1。
_C.RESNET.NUM_BLOCK_TEMP_KERNEL = [[3], [4], [6], [3]]

# 不同 ResNet 阶段的 stride 大小。
_C.RESNET.SPATIAL_STRIDES = [[1], [2], [2], [2]]

# 不同 ResNet 阶段的 dilation 大小。
_C.RESNET.SPATIAL_DILATIONS = [[1], [1], [1], [1]]

# ---------------------------------------------------------------------------- #
# X3D 选项
# X3D Networks 细节见 https://arxiv.org/abs/2004.04730。
# ---------------------------------------------------------------------------- #
_C.X3D = CfgNode()
...
# 是否在残差模块中间的 (3x3x3) conv 使用 channelwise (=depthwise) 卷积。
...
# 用于把 Fast pathway 信息融合到 Slow pathway 的 kernel 尺寸。
...
# 'exp' 和 'cos' 策略的 step size（单位：epoch）。
_C.SOLVER.STEP_SIZE = 1

# 'steps_' 策略的 steps（单位：epoch）。
_C.SOLVER.STEPS = []
...
# 训练用 box list 文件名。这里假设包含预测框的文件名会带有
# "predicted_boxes" 后缀。
...
# 计算 BN 统计量的 clip 数量。细节见 https://arxiv.org/abs/1912.00998。
...
# 写入摘要记录器；训练/评估期间会自动记录 loss、lr 和指标。
...
# 对每个选定真值标签，在直方图上可视化预测最多的 top-k 类。
...
# 如果为 True，使用每个实例的真值标签可视化 Grad-CAM。
...
# 要可视化的标签子集。只会可视化真值标签落在该子集中的错误预测。
...
# 每隔 `DEMO.NUM_CLIPS_SKIP` + 1 个 clip 取一个用于预测和可视化。
# 这会通过降低预测/可视化频率来加快演示速度。
# 如果为 -1，则取最近读取的 clip 进行可视化。该模式仅在
