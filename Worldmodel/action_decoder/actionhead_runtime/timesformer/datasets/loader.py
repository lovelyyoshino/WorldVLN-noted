...
def detection_collate(batch):
    """
        检测任务使用的 collate 函数。
        它会把不同样本的 bboxes、labels 和 metadata 沿第一维拼接，
        而不是堆叠成普通 batch 维。

        参数：
            batch (tuple or list): 需要合并的数据 batch。

        返回：
            (tuple): 合并后的检测数据 batch。
...
def shuffle_dataset(loader, cur_epoch):
    ...
    if isinstance(sampler, DistributedSampler):
        # 分布式采样器会根据 epoch 设置不同的 shuffle 顺序。
