...
class Ssv2(torch.utils.data.Dataset):
    """
    Something-Something v2 (SSV2) 视频加载器。构建后会从视频中采样片段。
    训练和验证阶段对每个视频随机采一个片段，并执行随机裁剪、缩放和翻转；
    测试阶段则会均匀采样多个片段，并采用确定性的空间裁剪。若宽大于高，
    取左/中/右裁剪，否则取上/中/下裁剪。
    """

    def __init__(self, cfg, mode, num_retries=10):
        """
                将 Something-Something V2 数据（帧路径、标签等）加载到给定
                Dataset 对象中。数据集可从 Something-Something 官网下载：
                引用/来源：https://20bn.com/datasets/something-something 。
                更多数据格式说明见 datasets/DATASET.md。
                参数：
                    cfg (CfgNode): 配置对象。
                    mode (string): 可选 `train`、`val` 或 `test`。
                        训练和验证模式从对应集合中读取数据，并为每个视频采样一个片段；
                        测试模式从测试集读取数据，并为每个视频采样多个片段。
                    num_retries (int): 从磁盘读取帧时的重试次数。
    ...
    def _construct_loader(self):
        ...
        # 测试阶段 self._num_clips > 1 时，对样本列表进行扩展。
